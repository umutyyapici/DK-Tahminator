...
def auto_predict():
    ...
    preds_df.to_csv(PREDICTIONS_PATH, index=False)
    json_path = PREDICTIONS_PATH.replace(".csv", ".json")
    preds_df.to_json(json_path, orient="records", force_ascii=False)
    print(f"[auto_predict] {len(by_date)} gün için joker işaretlendi, "
          f"predictions.csv/.json ve joker_locks.json güncellendi.")
